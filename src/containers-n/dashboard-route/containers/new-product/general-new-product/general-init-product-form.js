...
export function make_general_init_product_form() {
    return {
        ...
        sell_price: '',
        images: [],
        draft__short_description: EditorState.createEmpty(),
        draft__description: EditorState.createEmpty(),
        package_width: '',
        package_length: '',
        package_height: '',
        package_weight: '',
...
export function from_db_make_general_init_product_form(from_db_general_product) {
    return {
        ...
        draft__short_description: EditorState.createWithContent(
            convertFromRaw(JSON.parse(sr(from_db_general_product, ['short_description'])))
        ),
        draft__description: EditorState.createWithContent(
            convertFromRaw(JSON.parse(sr(from_db_general_product, ['description'])))
        ),
